...
function transformToRequire(propertiesToTransform) {
    return function convert(tag) {
        if (tag && tag.attributes && tag.attributes.length > 0) {
            // replace instances of url(#foo) in attributes
            tag.attributes.forEach(function (attr) {
                console.log(getAttributeIndex(tag, 'xlink:href'));
                if (propertiesToTransform[tag.tagName]) {
                    let firstChar = attr[1].charAt(0),
                        value = attr[1];
                    
                    if (firstChar === '.' || firstChar === '~') {
                        if (firstChar === '~') {
                            value = '"' + attr[1].slice(2)
                        }
                        attr[1] = 'require(' + value + ')';
                    }
...
function runTransform(tokens, configOverride) {
    ...
    if (Object.keys(config.transformToRequire).length > 0) transformations.push(transformToRequire(config.transformToRequire));
